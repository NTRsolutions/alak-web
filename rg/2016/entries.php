...
if($_GET['name'] == NULL)
{
	echo ' <script>window.location = "entries.php?name=Adit";</script> ';
}
...
$data = mysql_fetch_array(mysql_query("SELECT fName, lName, nick FROM wingmates2016 WHERE fName='{$query_name}'"));
$fname = $data['fName'];
$nick = $data['nick'];
$lname = $data['lName'];
...
<html lang="en">
  ...
  ';
  	$posts = mysql_query("SELECT name FROM entry2016 WHERE name='{$fname}'");
	$count = 0;
...
';
  	...
  	$query = mysql_query("SELECT fName, lName, nick FROM wingmates2016 WHERE 1");
  	while ($people = mysql_fetch_assoc($query))
  	{
		...
		$posts = mysql_query("SELECT name FROM entry2016 WHERE name='{$name1}'"); 	
		$count = 0;
		while($post = mysql_fetch_assoc($posts))
		{
			$count++;
...
if($fuck == 1)
{
	...
	$posts = mysql_query("SELECT name, friend, ans1, ans2, ans3, ans4, ans5, words FROM entry2016 WHERE name='{$fname}'");
