...
echo '<button class="button place-right">'.$username.'</button>';
    		...
    		if($_GET['action']=='newEntry') //************* NOTIFY WEB ADMIN FOR APPROVAL (CHECK ALL OTHER WEB ADMIN APPROVAL ALSO)
		{
			...
			{
				mysql_query("INSERT INTO pageActivities (paDate, paTitle, paContent, paFlag) VALUES ('{$date}','{$title}', '{$content}', '0')");
				header('Location: editactivities.php');
			}
			else
			{
				...
				&& in_array($extension, $allowedExts)) 
				{
					...
					{
						move_uploaded_file($_FILES["file"]["tmp_name"], "../images/activities/".$title.'.jpg');
						mysql_query("INSERT INTO pageActivities (paDate, paTitle, paContent, paFlag) VALUES ('{$date}','{$title}', '{$content}', '1')");
						header('Location: editactivities.php');
				  	}
				} 
				else 
                    ...
                    <div class="tile tripleF double-vertical transparent" style="height: auto">
                        <div class="tile-content">
                            <div class="panel no-border fg-white">
                                ...
                                <div class="panel-header bg-gray fg-white">New Entry for ACTIVITIES Page</strong></div>
...
<a href="editactivities.php"><button class="button" style="margin-right: 10px">Back</button></a></center>    
